package com.wind.carmanager.dialog;
...
public class IOSAlertDialog {
    ...
    public IOSAlertDialog builder() {
        ...
        // 调整dialog背景大小
        lLayout_bg.setLayoutParams(new FrameLayout.LayoutParams((int) (display.getWidth() * 0.70),
    ...
    // 重载函数 增加对ResId资源的支持
    public IOSAlertDialog setNegativeButton(int textId, final OnClickListener listener) {
        String text = context.getResources().getString(textId);
        showNegBtn = true;
        if ("".equals(text)) {
            btn_neg.setText("取消");
