...
import com.wind.carmanager.activity.TrackQueryActivity;
import com.wind.carmanager.base.BaseFragment;
import com.wind.carmanager.dialog.FenceCreateDialog;
import com.wind.carmanager.dialog.FenceOperateDialog;
import com.wind.carmanager.dialog.FenceSettingDialog;
...
import com.wind.mapapi.clusterutil.clustering.Cluster;
import com.wind.mapapi.clusterutil.clustering.ClusterItem;
import com.wind.mapapi.clusterutil.clustering.ClusterManager;
 ...
 */
public class ElectricFenceFragment extends BaseFragment implements View.OnClickListener,
        ClusterManager.OnClusterItemClickListener<ElectricFenceFragment.MyItem>,
        ClusterManager.OnClusterClickListener<ElectricFenceFragment.MyItem>,
        BaiduMap.OnMapClickListener, BaiduMap.OnMapLoadedCallback{
    private static final String TAG = "ElectricFenceFragment";
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        setOptionImageBackground(R.drawable.ic_control);
    ...
    private void init() {
        initListener();
        trackApp = (MyApplication) mActivity.getApplication();
        trackApp.entityName =  BaseInfoSPUtil.getInstance().getDeviceName(mActivity);
        Log.i(TAG, "trackApp.entityName = " + trackApp.entityName);
        fenceOperateDialog = new FenceOperateDialog(this);
        viewUtil = new ViewUtil();
        // 地图工具
        mapUtil = MapUtil.getInstance();
        mTextureMapView = mBaseView.findViewById(R.id.fence_mapView);
        mapUtil.init((TextureMapView) mBaseView.findViewById(R.id.fence_mapView));
        // 设置地图加载监听
        mapUtil.baiduMap.setOnMapLoadedCallback(this);

        mClusterManager = new ClusterManager<>(mActivity, mapUtil.baiduMap);
        ...
        switch (view.getId()) {
            ...
            case R.id.btn_activity_options:
                if (null == fenceSettingDialog) {
                    fenceSettingDialog = new FenceSettingDialog(mActivity, settingCallback);
                }
    ...
    public boolean onClusterItemClick(MyItem item) {
        ...
        fenceOperateDialog.showAtLocation(mActivity.findViewById(R.id.layout_top),
    ...
    public void onMapClick(LatLng latLng) {
        ...
        if (null == fenceCreateDialog) {
            fenceCreateDialog = new FenceCreateDialog(mActivity, createCallback);
        }
    ...
    private void initListener() {
        settingCallback = new FenceSettingDialog.Callback() {
            ...
            public void onFenceOperateCallback(FenceType fenceType, FenceShape fenceShape,
                ...
                ElectricFenceFragment.this.fenceType = fenceType;
                ElectricFenceFragment.this.fenceShape = fenceShape;
                ElectricFenceFragment.this.fenceName = fenceName;
                ElectricFenceFragment.this.vertexesNumber = vertexesNumber;
                switch (operateType) {
                    case R.id.btn_create_fence:
                        mapUtil.init(mTextureMapView);
                        mapUtil.baiduMap.setOnMapClickListener(ElectricFenceFragment.this);
                        break;
        ...
        createCallback = new FenceCreateDialog.Callback() {
            ...
            public void onSureCallback(double radius, int denoise, int offset) {
                ElectricFenceFragment.this.radius = radius;
                ElectricFenceFragment.this.denoise = denoise;
                ElectricFenceFragment.this.offset = offset;
        ...
        fenceListener = new OnFenceListener() {
            ...
            public void onDeleteFenceCallback(DeleteFenceResponse response) {
                viewUtil.showToast(mActivity, response.getMessage());
            ...
            public void onFenceListCallback(FenceListResponse response) {
                ...
                if (0 == response.getSize()) {
                    ...
                    viewUtil.showToast(mActivity, message.toString());

                    return;
                }

                viewUtil.showToast(mActivity, getString(R.string.fence_operate_caption));
